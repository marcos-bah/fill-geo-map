...
for i in range(rows):
    for j in range(cols):
        if(matriz_inter_rios[i, j] == data):
            print(matriz_inter_rios[i, j], '==', data, 'b = ', brother)
        if(matriz_inter_rios[i, j] == data & brother == 0):
            print(brother)
            brother = 1
            origem = [i+1, j+1]
        elif(matriz_inter_rios[i, j] == data & brother == 1):
            print(brother)
            brother = 0
            #print(search_brother(origem, [i, j], matriz_rio_errado))
        else:
            pass
