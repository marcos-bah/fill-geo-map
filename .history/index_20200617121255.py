        ...
        while(ctt < 20):
            ...
            for k in range(8):
                if(m_rio_correto[x+dx[k], y+dy[k]] == noData):  # verifica se dado é -9999
                    ...
                    pass
                # verifica se vizinho é referencia, se for ele sai e assume sua posição
                elif([x+dx[k], y+dy[k]] == [m_refer[i+1][1], m_refer[i+1][2]]):
                    print("Achei Referencia")
                    break
                # verifica se vizinho está na black-list e o ignora
                elif([x+dx[k]-1, y+dy[k]] in black_list):
                    pass
                else:  # agora se sabe que esse vizinho possui valor 1
                    array.append(1)  # adiciona 1 no vizinho
                    # adiciona sua posição na blacklist
                    black_list.append([x+dx[k]-1, y+dy[k]])
