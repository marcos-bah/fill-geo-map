...
def carve_point(m_rio_correto, m_refer):
    ...
    for i in range(len(m_refer)):
        ...
        print(x, y)
        for k in range(8):
            print(m_rio_correto[x+dx[k], y+dx[k]])
            if(m_rio_correto[x+dx[k], y+dx[k]] == float(noData)):
                print(x+dx[k], y+dx[k])
                array.append(0)
            else:
                array.append(1)
...
aux = np.array(array_pits(matriz_inter_rios, matriz_mapa_elevacao))
aux = pd.DataFrame(aux)

aux.to_csv('saida.csv')

print(carve_point(matriz_rio_correto, aux))
